<template>
    <div class="container" style="margin: 20px">
        <div class="search">
            <Input v-model="title" placeholder="培训名称" style="width: 200px" clearable/>
            <Select v-model="type" style="width:150px; margin-left: 20px" placeholder="培训类型" clearable>
        ...
        <Divider style="padding-top: 20px">培训列表</Divider>
        <div class="table">
            <Button type="success" icon="md-add-circle" @click="register">培训记录</Button>
        ...
        <div>
            ...
            <Modal
                ...
                <Form :model="formItem" :label-width="120">
                    <FormItem label="培训标题" required>
                        <Input v-model="formItem.title" placeholder="请输入培训标题" ></Input>
                    </FormItem>
                    <FormItem label="培训类型" required>
                        <Select v-model="formItem.type" style="width:200px" placeholder="培训类型" clearable>
                            <Option v-for="item in typeList" :value="item.value" :key="item.value">{{ item.label }}</Option>
                        </Select>
                    </FormItem>
                    ...
                    <divider orientation="center">培训编辑</divider>
...
<!--            <Modal
                ...
                <Form :model="resultItem" :label-width="120">
                    <FormItem label="培训标题" required>
                        <Input v-model="resultItem.title" placeholder="请输入培训标题" ></Input>
                    </FormItem>
                    <FormItem label="培训类型" required>
                        <Select v-model="resultItem.type" style="width:200px" placeholder="培训类型" clearable>
                            <Option v-for="item in typeList" :value="item.value" :key="item.value">{{ item.label }}</Option>
                        </Select>
                    </FormItem>
                    <FormItem label="记录人" required>
                        <Input v-model="resultItem.author" placeholder="请输入记录人" ></Input>
                    </FormItem>
                    <FormItem label="培训日期">
                        <DatePicker type="year" format="yyyy-MM-dd" placement="bottom-end" placeholder="选择日期" :value="resultItem.date" @on-change="statusDateChange" ></DatePicker>
                    </FormItem>
                    <divider orientation="center">培训编辑</divider>
                    <div style="border: 1px solid #ccc; padding: 20px">
                        <Toolbar
...
export default Vue.extend({
    ...
    methods: {
        ...
        submit() {
            ...
            request.post(
                '/personTrain/submitTrainingInfo',
                formData,
                {
                    headers: {
        ...
        remove() {
            ...
            request.delete(
                '/personTrain/deleteTrainingById',
                {
                    headers: {
                        'content-type': 'application/json;charset=UTF-8',
                    },
